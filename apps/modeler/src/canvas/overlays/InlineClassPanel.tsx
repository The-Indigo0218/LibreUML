/**
 * InlineClassPanel — docked contextual properties panel for the selected class (R9).
 *
 * The class-level counterpart of InlineEdgePanel: instead of opening the full
 * SSoTClassEditorModal for the frequent edits, this panel docks to the right of
 * the canvas and edits the class name + its attributes / operations live. Each
 * change is one undo entry (committed on blur / Enter / visibility toggle).
 *
 * Scope: CLASS only. Interfaces, enums, parameters, type-derived relations and
 * stereotypes stay in the modal, reachable via the "Advanced…" button.
 *
 ...
 *
 * Keyed by the parent on the class signature so an external change (undo, modal
 * edit) remounts it with fresh state; local typing does not.
 */
...
export interface InlineClassPanelProps {
  /** Element id of the class being edited (also used as part of the parent key). */
  elementId: string;
  /** Open the full modal editor (interfaces/enums, params, types, relations). */
  onAdvanced: () => void;
  /** Close the panel. */
  onClose: () => void;
}
...
export default function InlineClassPanel({ elementId, onAdvanced, onClose }: InlineClassPanelProps) {
  ...
  if (!activeModel || !cls) return null;

  const attributes = (cls.attributeIds ?? [])
    .map((id) => activeModel.attributes[id])
    .filter((a): a is IRAttribute => !!a);
  const operations = (cls.operationIds ?? [])
    .map((id) => activeModel.operations[id])
  ...
  /** Reads the freshest member arrays at commit time (avoids stale closures). */
  const readMembers = (): { attrs: IRAttribute[]; opsList: IROperation[] } => {
    const m = isStandalone && activeTabId ? getLocalModel(activeTabId) : useModelStore.getState().model;
    const c = m?.classes[elementId];
  ...
  const addAttr = (name: string) => {
    ...
    commitAttrs([
      ...readMembers().attrs,
      { id: crypto.randomUUID(), kind: 'ATTRIBUTE', name: n, type: 'String', visibility: 'private' },
    ]);
  };
  ...
  const addOp = (name: string) => {
    ...
    commitOps([
      ...readMembers().opsList,
      { id: crypto.randomUUID(), kind: 'OPERATION', name: n, returnType: 'void', visibility: 'public', parameters: [] },
    ]);
  };

  const rename = (name: string) => {
    const n = name.trim();
    if (n && n !== cls.name) ops.updateClass(elementId, { name: n });
  };
        ...
        <section className="space-y-1.5">
          ...
          {attributes.map((a) => (
            <div key={`${a.id}:${a.name}:${a.type}:${a.visibility}`} className="flex items-center gap-1">
              ...
              <button className={delBtnCls} title={t('inlineClassPanel.remove')} onClick={() => deleteAttr(a.id)}>
          ...
          <div className="flex items-center gap-1">
            ...
            <input
              placeholder={t('inlineClassPanel.addAttribute')}
              onKeyDown={onAddKey(addAttr)}
              onBlur={(e) => { addAttr(e.target.value); e.target.value = ''; }}
        ...
        <section className="space-y-1.5 border-t border-surface-border/50 pt-3">
          ...
          {operations.map((o) => (
            <div key={`${o.id}:${o.name}:${o.returnType}:${o.visibility}`} className="flex items-center gap-1">
              ...
              <button className={delBtnCls} title={t('inlineClassPanel.remove')} onClick={() => deleteOp(o.id)}>
                <X className="w-3.5 h-3.5" />
              </button>
            </div>
          ))}
          <div className="flex items-center gap-1">
            <Plus className="w-3.5 h-3.5 text-text-muted shrink-0" />
            <input
              placeholder={t('inlineClassPanel.addOperation')}
              onKeyDown={onAddKey(addOp)}
              onBlur={(e) => { addOp(e.target.value); e.target.value = ''; }}
              className={`${fieldCls} flex-1`}
            />
          </div>
        </section>
