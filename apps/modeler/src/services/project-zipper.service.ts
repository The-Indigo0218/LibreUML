...
import { JavaIRGeneratorService } from "./javaIRGenerator.service";
...
interface IRProjectConfig extends BaseProjectConfig {
  elementIds: string[];
  model: SemanticModel;
...
export class ProjectZipperService {
  ...
  static async generateAndDownloadZipFromModel(config: IRProjectConfig) {
    const zip = new JSZip();
    ...
    for (const elementId of config.elementIds) {
      const { model } = config;
      const element =
      ...
      const elementPackage = element.packageName || config.packageName;
      packageStats.set(elementPackage, (packageStats.get(elementPackage) || 0) + 1);

      const javaCode = JavaIRGeneratorService.generate(elementId, config.codeConfig, model);
      const packagePath = elementPackage.replace(/\./g, "/");
      const targetFolder = srcBaseFolder.folder(packagePath);
      if (!targetFolder) throw new Error(`Could not create folder for package: ${elementPackage}`);
      targetFolder.file(`${element.name}.java`, javaCode);
    }

    const mainPackagePath = config.packageName.replace(/\./g, "/");
    const mainFolder = srcBaseFolder.folder(mainPackagePath);
    if (mainFolder) {
      mainFolder.file("Main.java", `package ${config.packageName};\n\npublic class Main {\n    public static void main(String[] args) {\n        System.out.println("Project '${config.projectName}' generated by LibreUML");\n    }\n}`);
    }

    zip.file("README.md", this.generateReadme(config, config.elementIds.length, packageStats));
...
public class Main {
    ...
    const readmeContent = this.generateReadme(config, config.nodes.length, packageStats);
  ...
  private static generatePomXml(config: BaseProjectConfig): string {
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
  ...
  private static generateBuildGradle(config: BaseProjectConfig): string {
    return `plugins {
    id 'java'
    id 'application'
}
...
java {
  ...
  private static generateReadme(
    ...
    lines.push("");
    lines.push(`- **Total Classes:** ${classCount}`);
