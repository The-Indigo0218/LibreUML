...
import type { ActivityObjectNodeViewModel } from '../../adapters/view-models/node.view-model';
 ...
 * its own (ADR-0002) — same rule as `getActionShapeSize`, which this mirrors.
 */
export function getObjectNodeShapeSize(
  vm: ActivityObjectNodeViewModel,
...
interface ObjectNodeShapeProps {
  viewModel: ActivityObjectNodeViewModel;
  x: number;
  y: number;
  selected?: boolean;
  opacity?: number;
 ...
 * glance. The classifier trace (ADR-0010), when set, prints below the name
 * in brackets — `[ClassName]`, the standard UML object notation.
 */
export default function ObjectNodeShape({
  viewModel: vm,
  x,
  y,
  selected,
  ...
  onDragEnd,
  dragBoundFunc,
}: ObjectNodeShapeProps) {
  const { width: W, height: H } = getObjectNodeShapeSize(vm);
  const fontSize = vm.fontSizeOverride ?? LABEL_FONT;
  const fontFamily = vm.fontFamilyOverride ?? FONT_SANS;
