...
export default function ProjectStructure() {
  ...
  // Merge a standalone .luml file back into the shared project. Logic lives in the
  // reusable `addStandaloneToProject` action (shared with the Problems panel + save
  // CTA); here we just bridge it to the context-menu target.
  const handleAddStandaloneToProject = () => {
    if (!contextMenu) return;
    addStandaloneToProject(contextMenu.nodeId);
    setContextMenu(null);
  };
